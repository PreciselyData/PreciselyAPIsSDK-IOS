#import <Foundation/Foundation.h>
#import "PBObject.h"

/**
* Precisely APIs
* Enhance & enrich your data, applications, business processes, and workflows with rich location, information, and identify APIs.
*
* The version of the OpenAPI document: 14.0.0
* 
*
* NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
* https://openapi-generator.tech
* Do not edit the class manually.
*/


#import "PBEmployeeCount.h"
#import "PBZonesContactDetails.h"
#import "PBZonesParentBusiness.h"
#import "PBZonesPoiClassification.h"
@protocol PBEmployeeCount;
@class PBEmployeeCount;
@protocol PBZonesContactDetails;
@class PBZonesContactDetails;
@protocol PBZonesParentBusiness;
@class PBZonesParentBusiness;
@protocol PBZonesPoiClassification;
@class PBZonesPoiClassification;



@protocol PBZonesPoi
@end

@interface PBZonesPoi : PBObject


@property(nonatomic) NSString* _id;

@property(nonatomic) NSString* name;

@property(nonatomic) NSString* brandName;

@property(nonatomic) NSString* tradeName;

@property(nonatomic) NSString* franchiseName;

@property(nonatomic) NSString* open24Hours;

@property(nonatomic) PBZonesContactDetails* contactDetails;

@property(nonatomic) PBZonesPoiClassification* poiClassification;

@property(nonatomic) PBEmployeeCount* employeeCount;

@property(nonatomic) NSString* organizationStatusCode;

@property(nonatomic) NSString* organizationStatusCodeDescription;

@property(nonatomic) PBZonesParentBusiness* parentBusiness;

@property(nonatomic) NSString* tickerSymbol;

@property(nonatomic) NSString* exchangeName;

@end
