#import "PBAddressAutocompleteEnterpriseServiceApi.h"
...
@interface PBAddressAutocompleteEnterpriseServiceApi ()

@property (nonatomic, strong, readwrite) NSMutableDictionary *mutableDefaultHeaders;

@end

@implementation PBAddressAutocompleteEnterpriseServiceApi

NSString* kPBAddressAutocompleteEnterpriseServiceApiErrorDomain = @"PBAddressAutocompleteEnterpriseServiceApiErrorDomain";
NSInteger kPBAddressAutocompleteEnterpriseServiceApiMissingParamErrorCode = 234513;
...
-(NSURLSessionTask*) listLocationsWithSearchText: (NSString*) searchText
    ...
    if (searchText == nil) {
        ...
        if(handler) {
            ...
            NSError* error = [NSError errorWithDomain:kPBAddressAutocompleteEnterpriseServiceApiErrorDomain code:kPBAddressAutocompleteEnterpriseServiceApiMissingParamErrorCode userInfo:userInfo];
            handler(nil, error);
        }
        return nil;
    }
    ...
    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/typeahead/v1/list/locations"];
...
-(NSURLSessionTask*) listSelectWithSelectedAddress: (NSString*) selectedAddress
    ...
    if (selectedAddress == nil) {
        NSParameterAssert(selectedAddress);
        if(handler) {
            NSDictionary * userInfo = @{NSLocalizedDescriptionKey : [NSString stringWithFormat:NSLocalizedString(@"Missing required parameter '%@'", nil),@"selectedAddress"] };
            NSError* error = [NSError errorWithDomain:kPBAddressAutocompleteEnterpriseServiceApiErrorDomain code:kPBAddressAutocompleteEnterpriseServiceApiMissingParamErrorCode userInfo:userInfo];
            handler(nil, error);
        }
        return nil;
    ...
    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/typeahead/v1/list/select"];

    NSMutableDictionary *pathParams = [[NSMutableDictionary alloc] init];

    NSMutableDictionary* queryParams = [[NSMutableDictionary alloc] init];
    if (selectedAddress != nil) {
        queryParams[@"selectedAddress"] = selectedAddress;
    }
    if (country != nil) {
        queryParams[@"country"] = country;
    }
