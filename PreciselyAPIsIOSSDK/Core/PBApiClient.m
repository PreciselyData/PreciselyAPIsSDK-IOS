...
static NSString * const kPBContentDispositionKey = @"Content-Disposition";
...
static NSString * PB__fileNameForResponse(NSURLResponse *response) {
    ...
    if(!headers[kPBContentDispositionKey]) {
    ...
    NSString *contentDispositionHeader = headers[kPBContentDispositionKey];
...
+ (instancetype) sharedClient {
    static PBApiClient *sharedClient = nil;
...
- (instancetype)init {
    return [self initWithConfiguration:[PBDefaultConfiguration sharedConfig]];
}
...
- (instancetype)initWithBaseURL:(NSURL *)url configuration:(id<PBConfiguration>)configuration {
    ...
    if (self) {
        ...
        _timeoutInterval = 60;
        _responseDeserializer = [[PBResponseDeserializer alloc] init];
        _sanitizer = [[PBSanitizer alloc] init];

        _downloadTaskResponseTypes = @[@"NSURL*", @"NSURL"];

        AFHTTPRequestSerializer* afhttpRequestSerializer = [AFHTTPRequestSerializer serializer];
        PBJSONRequestSerializer * swgjsonRequestSerializer = [PBJSONRequestSerializer serializer];
        _requestSerializerForContentType = @{kPBApplicationJSONType : swgjsonRequestSerializer,
...
- (NSURLSessionTask*) requestWithPath: (NSString*) path
    ...
    __weak id<PBSanitizer> sanitizer = self.sanitizer;
